"""
Baseline MTD Controller (Fixed-Interval, Simultaneous Install)
==============================================================
Reproduces conventional MTD for comparison:
  - mutations fire on a fixed timer, not on threat detection
  - all switches are programmed at the SAME instant (stagger=False),
    producing the synchronised burst that fingerprinting tools detect

Run with:
  ryu-manager controller/baseline_controller.py

FIX: hosts are now registered with the mutation module as they are
learned, so mutations actually install flow rules.
...
class BaselineController(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.mutation_module = MutationModule(self.datapaths, stagger=False,
                                              port_resolver=self._resolve_port)
        self.mtd_trigger     = MTDTrigger(self.mutation_module, mode=MTD_MODE)

        logger.info("Baseline Controller started - fixed interval, simultaneous install")
